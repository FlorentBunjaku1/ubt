...
Select MIN(p.Sasia) as 'Min_i_porosive'
From Porosia p

Select AVG(p.CmimiPerNjesi) as 'Mesatarja_e_cmimit'
From Produkti p

Select DISTINCT k.vendbanimi
From Klienti k

Select po.PorosiaID, k.Emri, k.Mbiemri, pr.EmriProduktit, po.sasia
From Porosia po, Produkti pr, Klienti k
Where po.Produkti = pr.ID AND k.id = po.Klienti

Select k.Emri, k.Mbiemri, pr.EmriProduktit, po.Data, po.Sasia
From Porosia po, Produkti pr, Klienti k
Where po.Produkti = pr.ID AND k.id = po.Klienti AND (k.Emri+' '+k.Mbiemri) LIKE 'Gresa Gashi' 
...
Select k.Emri, k.Mbiemri, p.klienti, SUM(p.sasia) 'shuma_e_porosive'
From Klienti k, Porosia p
Where k.id = p.klienti
Group By k.Emri, k.Mbiemri, p.klienti


Select p.PorosiaID, pr.EmriProduktit, p.Klienti, p.sasia, pr.prodhuesi
From Porosia p, Produkti pr
Where p.Produkti = pr.ID AND pr.EmriProduktit = 'PC'


Select k.Emri, k.Mbiemri, p.klienti, count(*) 'Nr_Porosive'
From Klienti k, Porosia p
Where k.id = p.klienti
Group By k.Emri, k.Mbiemri, p.klienti


Select k.Emri, k.Mbiemri, sum(p.sasia) 'shuma_e_Porosive'
From Klienti k, Porosia p
Where k.id = p.klienti AND (k.Emri+' '+k.Mbiemri) LIKE 'Albin Krasniqi'
Group By k.Emri, k.Mbiemri, p.klienti


Select p.PorosiaID, k.Emri, k.Mbiemri, pr.Prodhuesi, p.Sasia, p.data
From Klienti k, Porosia p, produkti pr
Where k.id = p.klienti AND p.produkti = pr.ID AND (p.Sasia BETWEEN 5 AND 10) AND p.data BETWEEN '2020-10-10' AND '2020-10-14'

Select k.Emri, k.Mbiemri, p.Klienti, count(*) 'numri_i_porosive'
From Klienti k, Porosia p
Where k.id = p.klienti
Group By k.Emri, k.Mbiemri, p.Klienti
Having count(*) > 1

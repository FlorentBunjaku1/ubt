...
(
	...
	From Prodhuesi p JOIN Katalogu k ON p.prodhues_ID = k.prodh_ID JOIN Automobili a ON a.automobil_ID = k.a_ID
...
Alter View PK AS
(
	Select p.prodhues_ID, p.shteti_P
	From Prodhuesi p Left Join Katalogu k ON  p.prodhues_ID = k.prodh_ID
...
Create View MAXQMIMI AS
(
	Select MAX(k.qmimi_shitjes) as 'cmimi'
	From Prodhuesi p JOIN Katalogu k ON p.prodhues_ID = k.prodh_ID JOIN Automobili a ON a.automobil_ID = k.a_ID
	Where a.klasa = 'SUV'
)

Select p.emri_P, k.qmimi_shitjes
From Prodhuesi p JOIN Katalogu k ON p.prodhues_ID = k.prodh_ID JOIN Automobili a ON a.automobil_ID = k.a_ID
Where a.klasa = 'SUV' AND k.qmimi_shitjes = (Select * From MAXQMIMI)

/*4. Ne katalogun tuaj, gjeni prodhuesit nga te cilët posedoni automobila te klases ‘SUV’ por
jo edhe ‘Coupe’*/

Create View Coupe AS
(
	Select DISTINCT p.prodhues_ID
	From Prodhuesi p JOIN Katalogu k ON p.prodhues_ID = k.prodh_ID JOIN Automobili a ON a.automobil_ID = k.a_ID
	Where a.klasa = 'Coupe'
)
Create View SUV as
...
Select DISTINCT p.prodhues_ID, p.emri_P
From Prodhuesi p JOIN Katalogu k ON p.prodhues_ID = k.prodh_ID
Where p.prodhues_ID NOT IN(Select * From Coupe)


Select DISTINCT p.prodhues_ID, p.emri_P
From Prodhuesi p JOIN Katalogu k ON p.prodhues_ID = k.prodh_ID
Where p.prodhues_ID IN (Select * From Coupe) AND p.prodhues_ID IN (Select * From SUV)
...
With nrKerreve As
(
	Select TOP 2 p.emri_P, count(*) 'NumriKerreve'
	From Prodhuesi p JOIN Katalogu k ON p.prodhues_ID = k.prodh_ID JOIN Automobili a ON a.automobil_ID = k.a_ID
	Where a.klasa = 'Hatchback'
	Group By p.emri_P
	Order By 'NumriKerreve' DESC
)
Select DISTINCT p.emri_P, (Select MIN(nr.NumriKerreve) 
							From nrKerreve nr)'Numri i Kerreve'
From Prodhuesi p JOIN Katalogu k ON p.prodhues_ID = k.prodh_ID JOIN Automobili a ON a.automobil_ID = k.a_ID
Where a.klasa = 'Hatchback'
Group By p.emri_P
Having count(*) = (Select MIN(nr.NumriKerreve)
				   From nrKerreve nr)

...
Select p.Emri, p.Mbiemri, p.Datelindja, (YEAR(getdate())-YEAR(p.Datelindja)) as [Mosha]
From Pacienti p

/*2.Seletko 5 pacientët sipas emrit dhe mbiemrit, emri i të cilëve permban shkronjen N dhe R
ose shkronja e dyte e emrit eshte 'A*/
Select top 5 p.Emri, p.Mbiemri
From Pacienti p
Where p.Emri LIKE  '%n' or p.Emri LIKE '%r' or p.Emri LIKE '_a%'


/*3. Shtoni per 10% pagen e secilit doktor. */
Update Doktori 
set PagaMujore += (PagaMujore * 10/100);

Select * From Doktori


/*4. Selekto diagnozat e dhena nga doktori me id 7 dhe 12.*/
Select *
From Doktori d INNER JOIN Diagnoza_Doktori dd
	ON d.Id = dd.Doktori_id
	INNER JOIN Diagnoza di
	ON di.Diagnoza_ID = dd.Diagnoza_id
Where d.Id = 7 or d.Id = 12



/*5. Selekto spitalet te cilet nuk kan pacientë te shtrire*/
select s.Id, s.Emri, s.Vendi,p.Emri
From Spitali s left join Pacienti p
	ON p.Spitali = s.Id
Where p.Emri is null

/*6.Selekto spitalet te cilet kane nga 2 doktor.*/
Select s.id,s.Emri, COUNT(d.Spitali) [Nr_Doktorreve] 
From Spitali s inner join Doktori d
	On d.Spitali = s.Id
Group by s.id, s.Emri
HAVING COUNT(d.Spitali) = 2

/*7. Selekto te gjitha diagnozat te cilat nuk jane pershkruar nga ndonje doctor. (Hint: të
përdoret Inner ose Left/Right Join)*/Select di.Diagnoza_ID, di.Pershkrimi, di.Semundja, d.Id
From Doktori d right JOIN Diagnoza_Doktori dd
	ON d.Id = dd.Doktori_id
	 right JOIN Diagnoza di
	ON di.Diagnoza_ID = dd.Diagnoza_id
Where d.id is null


/*8. Selekto te gjithe spitalet te cilet kane me pak se 3 pacient. Selektimi te behet sipas emrit
dhe mbiemrit te pacientit si dhe emrit dhe vendit te spitalit. (Hint: të përdoret Left ose
Right Join)*/
Select s.Id, s.Emri, COUNT(*) [Nr i Pacienteve]
From Spitali  s inner join Pacienti p on s.Id = p.Spitali
Group By s.Id, s.Emri
Having COUNT(*) < 3


/*9. Listo të gjithë pacientët të cilët janë trajtuar në spitalin ‘Spitali i Mitrovices’, 'Rezonanca’
...
Inner Join)*/
Select p.Id, (p.Emri + ' ' + p.Mbiemri) as [Pacienti], (d.Emri + ' ' + d.Mbiemri) as [Doktori]
From Pacienti p inner join Spitali s on p.Spitali = s.Id inner Join Doktori d on d.Spitali = s.Id
Where s.Emri = 'Aloka' or s.Emri = 'Spitali i Mitrovices' or s.Emri = 'Rezonanca'
Order by s.Emri


/*10. Selekto te gjithe doktoret te cilet nuk kane pershkruar ndonje diagnose. (Hint: të përdoret
Left ose Right Join)*/Select d.Emri, d.Mbiemri, di.Diagnoza_ID
From Doktori d left JOIN Diagnoza_Doktori dd
	ON d.Id = dd.Doktori_id
	left JOIN Diagnoza di
	ON di.Diagnoza_ID = dd.Diagnoza_id
Where di.Diagnoza_ID is null


/*11. Selekto te gjithe pacientet sipas diagnozes se pershkruar nga doktoresha Eda Shala..
...
Select p.Id, p.Emri , p.Mbiemri, dk.Emri, dk.Mbiemri, d.Diagnoza_id, dg.Semundja
From Doktori dk inner join Diagnoza_Doktori d on d.Doktori_id = dk.Id 
	inner join Diagnoza dg on d.Diagnoza_id = dg.Diagnoza_ID 
	inner join Diagnoza_Pacienti dp on dp.Diagnoza_id = d.Diagnoza_id
	inner join Pacienti p on p.Id = dp.Pacienti_Id
Where dk.Emri like 'Eda' and dk.Mbiemri like 'Shala'

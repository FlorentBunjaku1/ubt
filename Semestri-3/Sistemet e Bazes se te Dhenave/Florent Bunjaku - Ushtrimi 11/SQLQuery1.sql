...
/*1. Duke përdorur views, në katalogun e kompanisë te shfaqen te gjithë automabilat të cilët
gjenden në katalog, janë prodhuar në Gjermani dhe kane ngjyren e zeze-Mat. Selektimi të
bëhet sipas Id dhe emrit të prodhuesit si dhe ngjyres së automobilëve.(Të përshkruhet query
gjithashtu përsmes algjebrës relacionale - simbolikisht) */
Create View KatalaoguAutomobili
AS
Select DISTINCT p.prodhues_ID, p.emri_P, a.ngjyra
From Automobili a JOIN Katalogu k ON a.automobil_ID = k.a_ID JOIN Prodhuesi p ON p.prodhues_ID = k.prodh_ID
Where p.shteti_p = 'Gjermani' AND a.ngjyra = 'zeze-Mat'


/*2. Duke përdorur views shfletoni prodhuesit te cilete nuk gjenden në katalog (Të përshkruhet
query gjithashtu përsmes algjebrës relacionale)*/
Create View KataloguA
AS
Select DISTINCT p.emri_P ,p.shteti_P
From Automobili a JOIN Katalogu k ON a.automobil_ID = k.a_ID Right JOIN Prodhuesi p ON p.prodhues_ID = k.prodh_ID
Where k.prodh_ID is null


Select p.emri_P, p.shteti_P
From Prodhuesi p 
Where p.prodhues_ID NOT IN ( Select k.prodh_ID
								From Katalogu k)


/*3. Gjeni cili prodhues ka çmimin me te larte për klasën SUV qe e kemi ne katalogun tonë.
(Të përshkruhet query gjithashtu përsmes algjebrës relacionale) */
Select DISTINCT  p.emri_P,a.klasa, a.qmimi_blerjes + k.qmimi_shitjes as 'Qmimi'
From Automobili a JOIN Katalogu k ON a.automobil_ID = k.a_ID JOIN Prodhuesi p ON p.prodhues_ID = k.prodh_ID
Where a.klasa = 'SUV' 


/*4. Ne katalogun tuaj, gjeni prodhuesit nga te cilët posedoni automobila te klases ‘SUV’ por
jo edhe ‘Coupe’. (Të përshkruhet query gjithashtu përsmes algjebrës relacionale) */
Select Distinct k.prodh_ID, p.emri_P
From Automobili a JOIN Katalogu k ON a.automobil_ID = k.a_ID JOIN Prodhuesi p ON p.prodhues_ID = k.prodh_ID
Where a.klasa NOT LIKE 'Coupe' AND k.prodh_ID  IN( Select  p.prodhues_ID
				From Automobili a JOIN Katalogu k ON a.automobil_ID = k.a_ID JOIN Prodhuesi p ON p.prodhues_ID = k.prodh_ID
				Where a.klasa = 'SUV')

Create View [Automobilat SUV jo Coupe] as
(
Select k.prodh_ID
From Katalogu k , Automobili a
Where k.a_ID = a.automobil_ID AND a.klasa = 'SUV'
except
Select k.prodh_ID
From Katalogu k, Automobili a
Where k.a_ID = a.automobil_ID AND a.klasa = 'Coupe'
)

select *
From [Automobilat SUV jo Coupe] suv, Prodhuesi p
Where suv.prodh_ID = p.prodhues_ID